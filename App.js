...
function App() {
  ...
  const contractAddress = "0x664BbDd398111267d8F24E78FF3D38890ba00f7D"; // Paste the address recieved from Remix IDE here
  const [wallet, setWallet] = useState(""); // Change1
  const [registeredCount, setRegisteredCount] = useState(0);  //Change2- Number of registered users
    ...
    const init = async () => {
      ...
      if (window.ethereum) {
        try {
          ...
          await getWalletAddress(); //Chnage 3- for updating wallet
  ...
  const getWalletAddress = async () => {
    try {
      ...
      setWallet(accounts[0]);  // Store the first account
    } catch (error) {
  ...
  const loadPeople = async () => {
    ...
    try {
      ...
      setRegisteredCount(peopleData.length);  //Change 4 - Save count
    } catch (error) {
      ...
      <header className="App-header">
        ...
        : (
          <div>
            ...
            <h3>People</h3>
            <p><strong>Total Registered Users:</strong> {registeredCount}</p>
